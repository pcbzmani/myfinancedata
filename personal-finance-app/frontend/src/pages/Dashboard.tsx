...
function buildDaily(txns: any[], period: number = 1) {
  ...
  return Object.entries(days).map(([, v], idx, arr) => {
    const [y, m, day] = arr[idx][0].split('-');
...
export default function Dashboard() {
  ...
  const daily   = buildDaily(txns, chartPeriod);
  // Show every Nth tick so x-axis isn't crowded (target ~6 labels)
